...
import { /* UNI_EXTENDED_LIST, UNI_LIST, */ UNSUPPORTED_LIST_URLS, ZORA_LIST } from './lists'
...
class TokenSafetyLookupTable {
  ...
  update(lists: ListsState) {
    ...
    // Initialize zora tokens first
    lists.byUrl[ZORA_LIST]?.current?.tokens.forEach((token) => {
