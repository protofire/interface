...
const CHAIN_SUBGRAPH_URL: Record<number, string> = {
  // Grahp URL Example
  // [UniverseChainId.ChainName]: 'https://graph.swap.w3us.site/subgraphs/name/{chain-name}/uniswap-v3',
}

const httpLink = new HttpLink({ uri: CHAIN_SUBGRAPH_URL[UniverseChainId.Mainnet] })
...
const authMiddleware = new ApolloLink((operation, forward) => {
  ...
  operation.setContext(() => ({
    uri: chainId && (CHAIN_SUBGRAPH_URL[chainId] ?? CHAIN_SUBGRAPH_URL[UniverseChainId.Mainnet]),
  }))
