...
interface AggregatorSwapCurrencyInputPanelProps {
  value: string
...
const AggregatorSwapCurrencyInputPanel = forwardRef<HTMLInputElement, AggregatorSwapCurrencyInputPanelProps>(
  (
    {
      value,
      onUserInput,
    ...
    const { tokens } = useEisenTokens(currentChainId)
    const [eisenIconUrl, setEisenIconUrl] = useState<string | undefined>(undefined)

    useEffect(() => {
      if (!currency) {
        setEisenIconUrl(undefined)
        return
      }

      // For native currency
      if (currency.isNative) {
        setEisenIconUrl('https://raw.githubusercontent.com/EisenFinance/assets/main/assets/icons/chains/flow/flow.png')
        return
      }

      // Find token in Eisen API tokens list
      const address = 'address' in currency ? currency.address : null
      if (address) {
        const token = tokens.find(t => t.address.toLowerCase() === address.toLowerCase())
        if (token && token.icon) {
          setEisenIconUrl(token.icon)
        } else {
          setEisenIconUrl(undefined)
        }
      } else {
                    ...
                    >
                      {!hideBalance && currency ? (
                        selectedCurrencyBalance ? (
                          ...
                          // Show 0 balance while loading or if balance is undefined
                          <Trans
                            i18nKey="swap.balance.amount"
                            values={{
                              amount: '0',
...
AggregatorSwapCurrencyInputPanel.displayName = 'AggregatorSwapCurrencyInputPanel'

export default AggregatorSwapCurrencyInputPanel
