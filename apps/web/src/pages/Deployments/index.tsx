...
const PageBackgroundReset = createGlobalStyle<{ $isDarkMode: boolean }>`
  html, body {
    background-color: ${({ $isDarkMode }) => ($isDarkMode ? '#131313' : '#fff9f8')} !important;
...
export default function DeploymentsPage() {
  ...
  return (
    <>
      <PageBackgroundReset $isDarkMode={isDarkMode} />
      <PageWrapper>
        ...
        <ChainList>
          {filteredChains.length === 0 ? (
            ...
            filteredChains.map((chain) => {
              ...
              const hasAddresses = Object.keys(chain.addresses).length > 0
